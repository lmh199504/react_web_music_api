	...
	if (file) {
		...
		if (result.res.status === 200) {
			const upData = await UserModel.findOneAndUpdate({
				...
				username: !newUsername ? username : newUsername.trim(),
				password: !newPassword ? findData.password : md5(newPassword),
				headerImg: result.url,
				isPrivate: !newPrivite ? findData.isPrivate : newPrivite
			})
			ctx.body = {
				code: 0,
				data: {
...
exports.addLoveSong = async (ctx, next) => {
	...
	const list = await LoveModel.find({
		userId
	})
	let num = 0
...
exports.delLoveSong = async (ctx, next) => {
	...
	for (let i = 0; i < delList.length; i++) {
		await LoveModel.findOneAndDelete({
			userId,
			songmid: delList[i].songmid
		})
		num++
	}
	ctx.body = {
...
exports.getLoveSong = async (ctx, next) => {
	...
	const findData = await LoveModel.find({
		userId: _id
	})
...
exports.addLoveSinger = async (ctx, next) => {
	...
	const findData = await LoveSingerModel.find({
		userId: _id
	})
	const index = findData.findIndex(item => item.singermid === singer.singermid)
	if (index === -1) {
		await new LoveSingerModel({
			userId: _id,
			userId: _id,
			...singer,
			orderId: _id + singer.singermid
		}).save()
		ctx.body = {
			code: 0,
			data: {
...
exports.delLoveSinger = async (ctx, next) => {
	...
	const findeData = await LoveSingerModel.find({
		userId: _id
	})
	...
	if (findeData === null) {
		...
		const delData = await LoveSingerModel.findOneAndDelete({
			userId: _id,
...
exports.addLoveSheet = async (ctx, next) => {
	...
	if (!username) {
		...
		const findData = await LoveSheetModel.findOne({
			userId: _id
		})
		if (!findData) {
			const saveData = await new LoveSheetModel({
				userId: _id,
				sheets: [{ ...sheet
				}]
			}).save()
			...
			await LoveSheetModel.findOneAndUpdate({
				userId: _id
			}, {
				sheets: [...sheets, sheet]
			})
...
exports.addUserSheet = async (ctx, next) => {
	...
	const usheets = await UserSheetModel.find({
		userId: _id
	})
	if (usheets) {
	...
	if (file !== '') {
		...
		const result = await client.put(catalog, Buffer.from(file, 'base64'));
		if (result.res.status === 200) {
			const saveData = await new UserSheetModel({
				userId: _id,
				desc,
				name,
				sheetId: sheetId,
				sheetCover: result.url
			}).save()
			ctx.body = {
				code: 0,
				data: {
...
exports.delUserSheet = async (ctx, next) => {
	...
	const delData = await UserSheetModel.findOneAndDelete({
		userId: _id,
		sheetId: sheetId
	})
	ctx.body = {
...
exports.addSongToSheet = async (ctx, next) => {
	...
	const findData = await SheetSongModel.find({
		userId: _id,
		sheetId: sheetId
	})
	let num = 0;
...
exports.getUserSheetSong = async (ctx, next) => {
	...
	if (!sheetId) {
		...
		const sheetData = await UserSheetModel.findOne({
			sheetId
		}, filter)
		const songList = await SheetSongModel.find({
			sheetId
		}, filter)
		ctx.body = {
			code: 0,
...
exports.uploadVideo = async (ctx, next) => {
	...
	if (!desc || !title) {
		ctx.body = {
			code: 10000,
			msg: 'desc or title is empty.'
		}
	}

	const files = ctx.request.files
	const filePath = files.files.path
	var temp = files.files.name.split('.');
	var fileType = temp[temp.length - 1];
	var lastName = '.' + fileType;
	// 构建图片名
	var fileName = username + '_' + Date.now() + lastName;
	const client = new OSS(ossConfig);
	var result = await client.put(`/uploadVideo/${fileName}`, filePath);
	try{
		var posterUrl = await createImg(filePath)
	}catch(e){
		//TODO handle the exception
		console.log("图片生成失败")
		var posterUrl = 'https://reactlmh.oss-cn-beijing.aliyuncs.com/images/ccd2a280-d652-11ea-8d08-bbc0b0f9d824.png'
	}
	const saveData = await new userUploadVideoModel({
		userId: _id,
		title,
		desc,
		url: result.url,
...
exports.getUserVideo = async (ctx, next) => {
	const {
		username
	} = ctx.session
	const {
	...
	const list = await userUploadVideoModel.find({
		userId: _id
	})
	ctx.body = {
		code: 0,
...
exports.delUserVideo = async (ctx, next) => {
	const {
		videoId
	} = ctx.request.body
	const {
	...
	await userUploadVideoModel.findOneAndDelete({
		userId: _id,
		_id: videoId
	})
	ctx.body = {
...
const createImg = (file) => {
	return new Promise((resolve,reject) => {
		//生成缩略图111
		/* 使用 fluent-ffmpeg 需要下载 其软件 添加至 path路径
		...
		*/
		...
		ffmpeg(file)
			...
			.on('end', async function() {
				...
				const client = new OSS(ossConfig);
				try{
					var result = await client.put(`/uploadVideoPoster/${time}.png`, `./poster/${time}.png`);
				}catch(e){
					reject(e)
				}
				resolve(result.url)
